defmodule Drivers.Supervisor do
  ...
  def init([driver_name]) do
    ...
    children = [
      worker(bus_module(configuration[:bus]), [configuration[:bus_name], configuration[:address], [name: driver_bus_name]]),
      worker(driver_module, [driver_bus_name, [name: driver_module]])
    ]
